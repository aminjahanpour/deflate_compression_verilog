...
always @(posedge bits_reader_finished_L) begin
    ...
    case (header_reader_stage)
        ...
        header_reader_stage_single_value  :       L1_decoder_main_loop_flag           = 1;
...
// flag
reg                                                     L1_decoder_main_loop_flag   = 0;
...
//L1_decoder_main_loop_flag
always @(negedge clk) begin
if (L1_decoder_main_loop_flag == 1) begin
...
if (L5_decode_ll_cl_cl_table_flag == 1) begin
    ...
    if (lagger_L5 == 1) begin
        ...
        decoder_cl_cl_ll_mem_write_addr = cl_cl_order[counter_L5];

        decoder_cl_cl_ll_mem_write_data = decoder_cl_cl_ll_mem_write_addr << 32;
        decoder_cl_cl_ll_mem_write_data[31 : 20] = (counter_L5 < cl_cl_count - hlit_cl) ? read_from_memory_bus_L : 0;

    end else if (lagger_L5 == 3) begin
        decoder_cl_cl_ll_mem_write_enable = 1;

    end else if (lagger_L5 == 4) begin
        decoder_cl_cl_ll_mem_write_enable = 0;

    end else if (lagger_L5 == 5) begin
        if ((decoder_cl_cl_ll_mem_write_data[31 : 20] > 0) && (decoder_cl_cl_ll_mem_write_data[31 : 20] < ll_cl_cl_min_L)) begin
            ll_cl_cl_min_L = decoder_cl_cl_ll_mem_write_data[31 : 20];
        end
        if (decoder_cl_cl_ll_mem_write_data[31 : 20] > ll_cl_cl_max_L) begin
            ll_cl_cl_max_L = decoder_cl_cl_ll_mem_write_data[31 : 20];
        end
...
if (L6_decode_distance_cl_cl_table_flag == 1) begin
    ...
    if (lagger_L6 == 1) begin
        ...
        decoder_cl_cl_distance_mem_write_addr = cl_cl_order[counter_L6];

        decoder_cl_cl_distance_mem_write_data = decoder_cl_cl_distance_mem_write_addr << 32;
        decoder_cl_cl_distance_mem_write_data[31 : 20] = (counter_L6 < cl_cl_count - hdist_cl) ? read_from_memory_bus_L : 0;


    end else if (lagger_L6 == 3) begin
        decoder_cl_cl_distance_mem_write_enable = 1;

    end else if (lagger_L6 == 4) begin
        decoder_cl_cl_distance_mem_write_enable = 0;

    end else if (lagger_L6 == 5) begin
        if ((decoder_cl_cl_distance_mem_write_data[31 : 20] > 0) && (decoder_cl_cl_distance_mem_write_data[31 : 20] < distance_cl_cl_min_L)) begin
            distance_cl_cl_min_L = decoder_cl_cl_distance_mem_write_data[31 : 20];
        end
        if (decoder_cl_cl_distance_mem_write_data[31 : 20] > distance_cl_cl_max_L) begin
            distance_cl_cl_max_L = decoder_cl_cl_distance_mem_write_data[31 : 20];
        end
...
if (L15_dump_ll_cl_cl_mem_flag == 1) begin
    ...
    if (lagger_L15 == 1) begin
        decoder_cl_cl_ll_mem_read_addr = counter_L15;

    // end else if (lagger_L15 == 2) begin
    //     if (counter_L15 < cl_cl_count) begin
    //     $display("counter_L15:%d, cl:%d", counter_L15, decoder_cl_cl_ll_mem_read_data[31 : 20]);
            
    //     end

    end else if (lagger_L15 == 3) begin
        $fdisplayb(L15_output_file_decoder_cl_cl_ll_mem, decoder_cl_cl_ll_mem_read_data);
        ...
        if (counter_L15 < max_length_symbol - 1) begin
            ...
            $fclose(L15_output_file_decoder_cl_cl_ll_mem);
            L15_dump_ll_cl_cl_mem_flag = 0;
            
            $display("L15: finished dumping decoder_cl_cl_ll at %d", $time);

            L1_decoder_main_loop_flag = 1;
...
if (L16_dump_distance_cl_cl_mem_flag == 1) begin
    ...
    if (lagger_L16 == 1) begin
        decoder_cl_cl_distance_mem_read_addr = counter_L16;

    // end else if (lagger_L16 == 2) begin
    //     if (counter_L16 < cl_cl_count) begin
    //     $display("counter_L16:%d, cl:%d", counter_L16, decoder_cl_cl_distance_mem_read_data[31 : 20]);
            
    //     end

    end else if (lagger_L16 == 3) begin
        $fdisplayb(L16_output_file_decoder_cl_cl_distance_mem, decoder_cl_cl_distance_mem_read_data);
        ...
        if (counter_L16 < max_length_symbol - 1) begin
            ...
            $fclose(L16_output_file_decoder_cl_cl_distance_mem);
            L16_dump_distance_cl_cl_mem_flag = 0;
            
            $display("L16: finished dumping decoder_cl_cl_distance at %d", $time);
